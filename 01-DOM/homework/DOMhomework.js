...
var selecID = document.querySelector('#createdBy');
selecID.innerHTML += 'Alejandro Serrano';
...
function buildToDos(toDos) {
  // Tu código acá:
  let array = toDos.map((todo, index) => {
...
function displayToDos() {
  ...
  for(let i = 0; i < resul.length; i++){
    toDoContainer.appendChild(resul[i]);
  }
}
...
// Tu código acá:
const botonAgrega = document.querySelector('#addButton');
botonAgrega.addEventListener('click', addToDo);
